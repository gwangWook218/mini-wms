...
- [x] **기존 상품 재고 입고(Inbound) API 개발 완료 (PATCH 활용)**
...
4. **객체지향 캡슐화를 통한 도메인 주도 설계**
    - 서비스 레이어에서 엔티티의 값을 외부에서 직접 수정(Setter 사용)하지 않도록 제한했습니다. 대신 `Product` 엔티티 내부에 `addQuantity()`라는 비즈니스 메서드를 정의하여 재고 연산의 주도권을 엔티티가 직접 갖도록 캡슐화했습니다.

5. **JPA 변경 감지(Dirty Checking) 적극 활용**
    - 입고 비즈니스 로직 처리 시 명시적인 `repository.save()` 호출을 지양하고, JPA 영속성 컨텍스트의 변경 감지 메커니즘을 활용했습니다. 이를 통해 트랜잭션 커밋 시점에 필요한 데이터만 안전하게 Update 쿼리가 발생하도록 최적화했습니다.